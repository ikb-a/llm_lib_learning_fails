...
Provenance = namedtuple("Provenance", ["creating_process", "creating_task", "inputs"])
...
def read_thm(path: str | os.PathLike) -> str:
    assert str(path).endswith(".thy"), str(path)
    with open(path, "r") as infile:
        return "".join(infile.readlines()).strip()
...
class SkillManager:
    def __init__(
        ...
        chroma_bridge: ChromaBridge | None = None,
        ...
        self.alt_thm_mode = alternative_theorems_list is not None
        if alternative_theorems_list is not None:
            assert isinstance(alternative_theorems_list, list), type(
                alternative_theorems_list
            )
            self.alt_theorems = [read_thm(x) for x in alternative_theorems_list]
    ...
    def add_new_problem(self, problem_name, formal_statement):
        raise RuntimeError("This function seems unused")
        data = (
            "problem_add_text",
            {
    ...
    def update_provenance(self, provenance: Provenance, name, statement, entity_type):
        """
        Assumes self.skill_manager_lock has been acquired! Unsafe to run otherwise.
        """

        provenance_dict = provenance._asdict()
        provenance_dict["created_type"] = entity_type
        provenance_dict["created_name"] = name
        provenance_dict["created_body"] = statement
        try:
            with open(f"{self.ckpt_dir}/skill/provenance.json", "r") as infile:
                prev_prov = json.load(infile)
        except FileNotFoundError:
            prev_prov = []
        prev_prov.append(provenance_dict)
        with open(f"{self.ckpt_dir}/skill/provenance.json.tmp", "w") as outfile:
            json.dump(prev_prov, outfile, sort_keys=True, indent=2)

        os.replace(
            f"{self.ckpt_dir}/skill/provenance.json.tmp",
            f"{self.ckpt_dir}/skill/provenance.json",
        )
    ...
    def add_new_request(
        ...
        with self.skill_manager_lock:
            ...
            self.update_provenance(
                provenance, request_name, formal_statement, entity_type="request"
            )
    ...
    def add_new_skill(
        ...
        with self.skill_manager_lock:
            ...
            self.update_provenance(
                provenance, skill_name, full_code, entity_type="skill"
            )
    ...
    def retrieve_skills(self, query, k):
        ...
        if self.alt_thm_mode:
            # truncate the list of k retrieved skills if needed, and load up to input_k
            # Limit system to at most half of the retrieved being from the library
            assert input_k // 2 > 0
            if len(ret_skill) > input_k // 2:
                ret_skills = random.sample(ret_skill, input_k // 2)
            alt_theorems = random.sample(self.alt_theorems, input_k - len(ret_skills))
            # Note: skills.json often has "-" as the skill description.
            alt_theorems = [
                {
                    "skill": "RANDOM_ALT_THEOREM",
                    "description": "-",
                    "code": thm_code,
                    "marker": thm_code,
                }
                for thm_code in alt_theorems
            ]
            ret_skill = alt_theorems + ret_skill
            random.shuffle(ret_skill)
    ...
    def retrieve_skills_with_context(self, context):
        ...
        input_k = 6  # Hard coded value of 6 from the original paper
