...
def extract_needed(proof_steps, current_step_index, needed_found):
    ...
    if search_index is None:
        ...
        return (
            extract_needed(proof_steps, search_index, needed_found)
            + [search_index]
            + sibling_indices
        )
    elif search_index == 0:
...
def extract_needed_string(
    transition,
    transitions_for_a_problem,
    ...
    needed_segment = " \\n ".join(
        [transitions_for_a_problem[index][1] for index in needed_indices]
    )
    return f"<ISA_NDS> {needed_segment} <ISA_OBS> {transition[0]}"


def extract_last_k_string(
    transition,
    transitions_for_a_problem,
    previous_proof_segment,
    i,
...
def extract_proof_and_state_string(
    transition,
    transitions_for_a_problem,
    previous_proof_segment,
    i,
...
def extract_trimmed_proof_and_state_string(
    transition,
    transitions_for_a_problem,
    previous_proof_segment,
    i,
...
def process_translations_for_a_problem(
    transitions_for_a_problem, processing_method_config=None
):
    ...
    for i in range(1, len(transitions_for_a_problem)):
        ...
        translation_src = ""
        if (
            processing_method_name is None
            or not processing_method_name in all_processing_methods
        ):
        ...
        translation_src += processing_method(
            transition,
            transitions_for_a_problem,
            previous_proof_segment,
            i,
...
def random_split_file_names(file_names, val_test_files=100):
    ...
    return (
        file_names[: -2 * val_test_files],
        file_names[-2 * val_test_files : -val_test_files],
        file_names[-val_test_files:],
    )


def process_files_with_proof_statements(
    file_names, saving_directory, processing_method_config=None
):
    # Store everything in jsonl format, and in text src and tgt format
    datapoints = {"train": list(), "valid": list(), "test": list()}
    problem_names_split = {"train": list(), "val": list(), "test": list()}
    ...
    for json_split in tqdm(datapoints.keys(), desc="Saving files"):
        ...
        with open(os.path.join(saving_directory, f"{split}.src"), "w") as f_src, open(
            os.path.join(saving_directory, f"{split}.tgt"), "w"
        ) as f_tgt:
            for datapoint in datapoints[json_split]:
    ...
    json.dump(
        problem_names_split,
        open(os.path.join(saving_directory, "problem_names_split.json"), "w"),
    )
    ...
    for split, lines in problem_names_split.items():
        ...
        with open(
            os.path.join(saving_directory, f"split.std_and_afp.{split}"), "w"
        ) as fout:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--extraction-file-directory",
        "-efd",
        nargs="+",
        help="Where the parsed json files are",
    )
    parser.add_argument(
        "--saving-directory", "-sd", help="Where to save the translation pairs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--last-k", "-lk", type=int, default=1, help="How many last k steps to use"
    )
    ...
    for extraction_file_directory in args.extraction_file_directory:
        file_names += list(
            glob.glob("{}/*/*_ground_truth.json".format(extraction_file_directory))
        )
