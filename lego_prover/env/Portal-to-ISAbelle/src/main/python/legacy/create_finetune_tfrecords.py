...
def parse_args():
    ...
    shuffle_pack_args.add_argument(
        "--n-repack-epochs", type=int, default=1, help=repack_ep_help
    )
    shuffle_pack_args.add_argument(
        "--seed",
        type=int,
...
def prep_and_tokenize_generator(
    string_iterable,
    encoder: TokenizerWrapper,
    normalize_with_ftfy,
...
def file_to_tokenized_docs_generator(file_path, encoder, args):
    ...
    token_list_gen = prep_and_tokenize_generator(
        string_iterable,
        encoder,
...
def arrays_to_sequences_pad(
    ...
    for chunk in tqdm(token_list_iterable):
        ...
        assert n_sep_tokens == n_eos_tokens, print(n_sep_tokens, n_eos_tokens)
...
def chunk_and_finalize(arrays, args, encoder):
    ...
    if seq2seq:
        ...
        sequences = list(
            map(lambda x: np.array(x, dtype=np.uint16), arrays_to_sequences(arrays))
        )
    ...
    if args.min_unique_tokens > 0:
        full_seqs = list(
            enforce_min_unique(full_seqs, args.min_unique_tokens, encoder, args.verbose)
        )
...
def create_tfrecords(files, args):
    ...
    if getattr(args, "seq2seq", True):
        ...
        assert all(encoder.decode(x[:20]).strip().startswith("<") for x in full_seqs)
        sep_id = encoder.sep_token_id
        eos_id = encoder.eos_token_id
        pad_id = encoder.pad_token_id
        for seq in full_seqs:
            last_non_pad_idx = max(i for i in range(len(seq)) if seq[i] != pad_id)
