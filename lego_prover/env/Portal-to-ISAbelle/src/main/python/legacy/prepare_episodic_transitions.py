...
def process_files_with_proof_statements(file_names, saving_directory):
    ...
    for file_path in tqdm(file_names):
        ...
        for problem_name in set(file["problem_names"]):
            ...
            episodic_transitions = process_translations_for_a_problem(
                transitions_split[problem_name]
            )
            unique_id_to_transitions[(original_file_name, problem_name)] = (
                episodic_transitions
            )

    for split, split_uids in problem_names_split.items():
        with jsonlines.open(
            os.path.join(saving_directory, "{}.jsonl".format(split)), "w"
        ) as writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--extraction-file-directory", "-efd", help="Where the parsed json files are"
    )
    parser.add_argument(
