...
def process(src_path, tgt_path, output_path, mode):
    ...
    with open(src_path) as src_fhand, open(tgt_path) as tgt_fhand, open(
        ...
        for index in tqdm(random_order):
            ...
            if mode == "state_only":
                output_fhand.write(
                    src_line.strip().replace("State:", "<ISA_OBS>")
                    + " Cambridge "
                    + tgt_line.strip()
                    + " <|endoftext|> "
                )
                ...
                output_fhand.write(
                    src_line.strip().replace("Proof:", "<ISA_PRF>")
                    + " Cambridge "
                    + tgt_line.strip()
                    + " <|endoftext|> "
                )
                ...
                output_fhand.write(
                    src_line.strip()
                    .replace("<PS_SEP> State:", "<ISA_OBS>")
                    .replace("Proof:", "<ISA_PRF>")
                    + " Cambridge "
                    + tgt_line.strip()
                    + " <|endoftext|> "
                )
...
def create_data(data_dir, output_dir, name, mode):
    ...
    train_output_path = os.path.join(output_dir, "train", "{}_train.txt".format(name))
    val_src_path = os.path.join(data_dir, "val.src")
    val_tgt_path = os.path.join(data_dir, "val.tgt")
    val_output_path = os.path.join(output_dir, "val", "{}_val.txt".format(name))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/home/qj213/data",
        help="Output directory (default: home data directory)",
    )
    parser.add_argument(
        "--name", type=str, help="Name of the currently processed data."
    )
    parser.add_argument(
        "--mode",
        type=str,
