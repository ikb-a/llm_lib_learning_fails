...
def evaluate_single_problem(
    ...
    state_string = stub.IsabelleCommand(
        server_pb2.IsaCommand(command="proceed:" + theorem_name)
    ).state
    ...
    try:
        for i in range(maximum_number_of_steps):
            ...
            # print(input_string)
            # print(output_string)
            state = stub.IsabelleCommand(
                server_pb2.IsaCommand(command=output_string)
            ).state
            # print(state)
...
if __name__ == "__main__":
    ...
    for i in range(0, 5):
        theory_file_path = train_names[i][0].replace(
            "/home/ywu/afp-2021-02-11", "/Users/qj213/Projects/afp-2021-02-11"
        )
