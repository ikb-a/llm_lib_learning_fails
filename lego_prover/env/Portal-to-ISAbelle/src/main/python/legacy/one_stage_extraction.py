...
def analyse_whole_file(whole_file_string, use_sledgehammer=False):
    ...
    for transition in transitions:
        ...
        if action.startswith("lemma") or action.startswith("theorem"):
            ...
            state_action_proof_level_tuples.append(
                (state, action, proof_level, hammer_results)
            )
            proof_open = True
        elif proof_open:
            state_action_proof_level_tuples.append(
...
def isa_step(stub, theory_file_path, use_sledgehammer=False):
    ...
    extraction_command = (
        "PISA extract data with hammer" if use_sledgehammer else "PISA extract data"
    )
...
def extract_file(
    ...
    close_program = False
    try:
        whole_file_parsed = isa_step(
            stub, theory_file_path, use_sledgehammer=use_sledgehammer
        )
        ...
        with open(
            os.path.join(
                ...
                "project_{}_file_{}_bug_report.txt".format(
                    working_directory.split("/")[-1], theory_file_path.split("/")[-1]
                ),
            ),
            "w",
        ) as fout:
    ...
    json.dump(
        ...
        open(
            os.path.join(
                saving_directory,
                "_".join(theory_file_path.split(".thy")[0].split("/"))
                + "_ground_truth.json",
            ),
            "w",
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--isa-path",
        help="The path to the Isabelle executable",
    ...
    parser.add_argument(
        "--saving-directory", "-sd", help="Where the save the parsed json files"
    )
    parser.add_argument(
        "--port", "-p", help="Port to use to communicate", default=9000, type=int
    )
    parser.add_argument(
        "--use-sledgehammer",
        "-us",
        help="Whether to use sledgehammer",
        action="store_true",
    )
    ...
    extract_file(
        args.isa_path,
        args.theory_file_path,
        args.working_directory,
        args.saving_directory,
        args.port,
        args.use_sledgehammer,
    )
