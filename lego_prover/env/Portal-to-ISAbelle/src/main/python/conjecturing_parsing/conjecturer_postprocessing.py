...
train_theorem_names = {
    process_string(element[1]): element[0] for element in train_names
}
...
for file_name in tqdm(glob.glob("data/conjecturer_extractions/**/*.src")):
    ...
    for i, line in enumerate(open(file_name).readlines()):
        theorem_name = (
            line.split("Proof:")[1].split("State:")[0].split("\\n")[0].strip()
        )
...
for i in range(3000):
    json_to_save = [[test_theorem_names[parsed_test_names[i]], parsed_test_names[i]]]
    json.dump(
        json_to_save, open("universal_test_theorems/test_name_{}.json".format(i), "w")
    )

    if i < 300:
        json.dump(
            json_to_save,
            open("universal_test_theorems/quick_test_name_{}.json".format(i), "w"),
        )
