...
def filter_comments(input_text, opening_marks, closing_marks):
    output_text = re.sub(
        re.compile("{}.*?{}".format(opening_marks, closing_marks), re.DOTALL),
        "",
        input_text,
    )
    return output_text
...
def spit_out_first_parsable_segment(input_text: str):
    ...
    if "theorem" in first_line or "lemma" in first_line:
        ...
        if (
            find_first_proof != -1
            and find_first_proof < find_first_apply
            and find_first_proof < find_first_by
        ):
            truncation_index = find_first_proof
        elif (
            find_first_apply != -1
            and find_first_apply < find_first_proof
            and find_first_apply < find_first_by
        ):
            truncation_index = find_first_apply
        elif (
            find_first_by != -1
            and find_first_by < find_first_proof
            and find_first_by < find_first_apply
        ):
            truncation_index = find_first_by
        else:
            raise NotImplementedError(
                "We haven't thought of this situation. The Isar proof is as follows:\n"
                + input_text
            )
    ...
    return (
        first_line.strip()
        + " "
        + input_text[find_first_line_separator:][
            : find_first_double_quotation + 1
        ].strip(),
...
def spit_out_all_parsable_segments(input_text: str):
    ...
    while rest_of_the_text:
        first_segment, rest_of_the_text = spit_out_first_parsable_segment(
            rest_of_the_text
        )
