...
with open(os.path.join(path_to_dir, "val.src")) as f1, open(
    os.path.join(path_to_dir, "val.tgt")
) as f2:
...
def preprocess(cmd, state_only):
    if state_only:
        ...
        returned_cmd = (
            cmd.replace("Proof:", "[IS] PROOF")
            .replace("State:", "[IS] GOAL")
            .replace("\\", "\\\\")
            .replace("\n", "\\n")
            .replace('"', '\\"')
            .replace("'", "\\u0027")
