...
script = (
    f"unset http_proxy; unset https_proxy; python3 src/main/python/one_stage_extraction.py  --isa-path {home_directory}/Isabelle2021 -wd {home_directory}"
    + "/afp-2021-10-22/thys/{} --saving-directory afp_extractions/{} -tfp {}"
)
if use_hammers:
    script = script + " -us"
...
total_files = 0
for project_name in glob.glob(
    "{}/afp-2021-10-22/thys/**/*.thy".format(home_directory), recursive=True
):
    ...
    if os.path.isfile(
        "afp_extractions/{}/{}_ground_truth.json".format(
    ...
    if (
        project_single_name not in IGNORED_ENTRIES
        and project_single_name + "/" + file_single_name not in IGNORED_THEORIES
    ):
        cmds.append(
...
for j, port in enumerate(ports):
    ...
    for i, cmd in enumerate(port_cmds):
        with open(f"scripts/extraction/script_{j}.sh", "a") as f:
            f.write(
                'unset http_proxy; unset https_proxy; sbt "runMain pisa.server.PisaOneStageServer{}"&\n'.format(
                    port
                )
            )
            f.write("PIDmain=$!\n")
