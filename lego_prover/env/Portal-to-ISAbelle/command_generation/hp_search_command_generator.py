...
if __name__ == "__main__":
    number_of_processes = input(
        "Enter the number of processes you want to run at the same time:\n"
    ).strip()
    ...
    use_state_first = input(
        "Want to use the state-only agent for the first step? (true/false) (false recommended)\n"
    ).strip()
    engine_id = input(
        "What engine id to use? (starting https: and ending with completions) (blank and will use small model by default)\n"
    ).strip()
    if len(engine_id) == 0:
        engine_id = ""
    ...
    for process_number, process_cmds in process_number_to_cmds.items():
        with open(
            "scripts/eval_search_conj_{}_use_proof_{}_use_state_first_{}_search_width_{}_maximum_queue_length_{}_temperature_{}_max_tokens_{}_max_trials_{}_timeout_{}_{}.sh".format(
                use_conj,
                use_proof,
                use_state_first,
                search_width,
                "sweep_{}".format("_".join([str(element) for element in mql_sweep])),
                temperature,
                max_tokens,
                max_trials,
                timeout,
