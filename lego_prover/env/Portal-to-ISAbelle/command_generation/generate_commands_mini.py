...
script = (
    f"python3 src/main/python/one_stage_extraction.py  --isa-path {home_directory}/Isabelle2021 "
    f"-wd {home_directory}"
    + "/afp-2021-10-22/thys/Symmetric_Polynomials --saving-directory mini_extractions/{} -tfp {}"
)
n_threads = 1
...
for project_name in glob.glob(
    "{}/miniF2F/isabelle/**/*.thy".format(home_directory), recursive=True
):
    ...
    print(
        "mini_extractions/{}/{}_ground_truth.json".format(
    ...
    if (
        ...
        cmds.append(script.format(project_single_name, project_name))
        total_files += 1
    ...
    with open("mini_extract_script_{}.sh".format(port), "w") as f:
        ...
        for i, cmd in enumerate(port_cmds):
            if (i + 1) % n_threads == 0:
                ...
                f.write(
                    'sbt "runMain pisa.server.PisaOneStageServer{}" &\n'.format(port)
                )
                f.write("PIDmain=$!\n")
                f.write("sleep 12\n")
            else:
