...
if __name__ == "__main__":
    ...
    last_k = input(
        "How many last steps to incldue as context (>=0, 0 to disable)?\n"
    ).strip()
    ...
    for process_number, process_cmds in process_number_to_cmds.items():
        with open(
            "scripts/eval_search_conj_{}_use_proof_{}_use_state_first_{}_search_width_{}_maximum_queue_length_{}_temperature_{}_max_tokens_{}_max_trials_{}_timeout_{}_usingT5_{}_greedy_{}_last_{}_needed_{}_{}.sh".format(
                use_conj,
                use_proof,
                use_state_first,
                search_width,
                "sweep_{}".format(mql_sweep),
                temperature,
                max_tokens,
                max_trials,
                timeout,
