...
if __name__ == "__main__":
    ...
    isabelle_identifier = (
        isabelle_identifier[-1] if isabelle_identifier[-1] else isabelle_identifier[-2]
    )
    ...
    for index in tqdm(range(args.number_of_copies)):
        ...
        shutil.copytree(
            args.isabelle,
            os.path.join(main_isa_path, isabelle_identifier),
            symlinks=True,
        )
        ...
        isabelle_settings_path = os.path.join(
            main_isa_path, isabelle_identifier, "etc/settings"
        )
        with open(isabelle_settings_path, "r") as f:
            settings = f.read()
        settings = settings.replace(
            original_isabelle_home_user_string, isabelle_home_user_string
        )
