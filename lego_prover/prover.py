...
from .agents import SkillManager, Provenance
...
def _formal_statement_in_proof(statement: str, proof: str) -> bool:
    """Checks if a formal statement appears in a proof generated by an LLM.

    Checks for direct inclusion, inclusion after changing all whitespace to a
    single space character, or inclusion after stripping comments.
    """
    # Try exact match
    if statement in proof:
        return True

    statement = statement.strip()
    proof = proof.strip()

    # If not, try removing comments & retrying
    comment_pattern = re.compile(r"\(\*[\s\S]*?\*\)")
    statement = re.sub(comment_pattern, "", statement)
    proof = re.sub(comment_pattern, "", proof)
    if statement in proof:
        return True

    # If not, standardize whitespace & re-check for inclusion
    whitespace_pattern = re.compile(r"\s+")
    statement = re.sub(whitespace_pattern, " ", statement)
    proof = re.sub(whitespace_pattern, " ", proof)
    if statement in proof:
        return True

    # Try replacing some usual suspects
    for operator in ["=", "*", "+", "-", "/", "<", ">", "^"]:
        proof = proof.replace(f" {operator} ", operator)
        statement = statement.replace(f" {operator} ", operator)
    if statement in proof:
        return True

    # Try stripping all whitespace before or after these symbols
    for symbol in ["::", '"', "(", ")"]:
        proof = proof.replace(f" {symbol}", symbol)
        proof = proof.replace(f"{symbol} ", symbol)
        statement = statement.replace(f" {symbol}", symbol)
        statement = statement.replace(f"{symbol} ", symbol)
    if statement in proof:
        return True

    # Rename any theorems to lemmas (keywords are equivalent in Isabelle)
    proof = proof.replace("lemma ", "theorem ")
    statement = statement.replace("lemma ", "theorem ")
    if statement in proof:
        return True
...
class Prover:
    def __init__(
        ...
        # filenames for the decomposer prompts
        ...
        # TODO: Doesn't seem to be used, commented out for now.
        ...
        self.action_agent = ActionAgent(
            ...
            decomposer_filename=decomposer_filename,
            formalizer_filename=formalizer_filename,
    ...
    def reset(self, task, context, reset_env=True):
        ...
        for request in skill_requests:
            ...
            provenance = Provenance(
                creating_process="prover_decomposer",
                creating_task=task,
                inputs=[context["problem_name"]],
            )
            self.skill_manager[U.problem_id(task)].add_new_request(
                problem_name=task,
                formal_statement=request,
                provenance=provenance,
                init_update_count=0,
            )
        ...
        return self.messages, skills

    def close(self):
        self.env.close()

    def step(self, task, reset_retrieved_skills):
        ...
        while n_retry > 0:
            try:
                ...
                assert _formal_statement_in_proof(
                    self.context["formal_statement"], parsed_result
        ...
        if isinstance(parsed_result, str):
            ...
            for req in requests:
                provenance = Provenance(
                    creating_process="prover_formalizer",
                    creating_task=task,
                    inputs=reset_retrieved_skills,
                )
                self.skill_manager[U.problem_id(task)].add_new_request(
        ...
        while True:
            try:
                if datetime.now() > self.end_time:
                    self.logger.info("Exceeded max job runtime; exiting prover")
                    break
                ...
                if context is None:
                    self.logger.info(
                        "Learning (presumably?) complete as context is None; exiting loop"
                    )
                    break
                ...
                if consecutive_failures >= 6:
                    raise
                self.env.reset(hard_reset=True)
                time.sleep(60)  # wait for prior programme to init
