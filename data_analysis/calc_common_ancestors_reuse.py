"""
Plot individual soft reuse lemma & task survival functions,
and record similarity data.
...
def process_run(datapath, run_idx, output_path, unrelated_lemmas):
    ...
    # Load lemmas that are not descended from a solution
    ...
    for task in parents:
        lemmas = parents[task]
        for lemma in lemmas:
            sim = similarity(lemma, answers[task])
            if len(children[lemma]) > 1:  # reused:
                reuse_sim.append(sim)
            else:
                single_use_sim.append(sim)
    ...
    for threshold in thresholds:
        # print(theshold_filtered_children)
    ...
    plt.ylabel("% of Non-Retrieved Lemmas Meeting Similarity Threshold in 2 Solutions")
    ax = plt.gca()
    ax2 = ax.twinx()
    ax2.plot(
    ...
    plt.savefig(os.path.join(output_path, f"lemma_count_sim_thres_{run_idx}.png"))
    plt.close()
    ...
    _, name = os.path.split(output_path)
    plt.title(name)
    plt.savefig(os.path.join(output_path, f"task_count_sim_thres_{run_idx}.png"))
    plt.close()
...
def main(data_paths, output_path, unrelated_lemmas, amortized_data_name: str):
    ...
    if len(amortized_data) == 0:
        single_use_sim, reuse_sim, non_retrieved_sim = [], [], []
        thresholds = None
        reused_lemmas_count = []
        task_with_reuse_count = []
        num_successes = []
        ...
        for i, datapath in enumerate(data_paths):
            ...
            # num_successes += znum_success
            num_successes.append(znum_success)

            single_use_sim.append(zsingle_use_sim)
            reuse_sim.append(zreuse_sim)
        ...
        amortized_data["single_use_sim"] = single_use_sim
        amortized_data["reuse_sim"] = reuse_sim
    ...
    single_use_sim = amortized_data["single_use_sim"]
    reuse_sim = amortized_data["reuse_sim"]
    ...
    # Flatten to single array and plot histogram
    single_use_sim = flatten(single_use_sim)
    reuse_sim = flatten(reuse_sim)
    non_retrieved_sim = flatten(non_retrieved_sim)

    # ===========================================================
    # ===========================================================
    # Repeat plots, this time with standard deviations over 3 runs

    reused_lemma_count_mean = np.array(reused_lemmas_count).mean(axis=0).flatten()
    plt.plot(thresholds, reused_lemma_count_mean)
    plt.xlabel("Similarity Threshold")
    plt.ylabel("Retrieved Lemmas reused in 2 tasks")
    ax = plt.gca()
    ax.axvline(np.average(non_retrieved_sim), color="red", linestyle="--")
    reused_lemma_count_std = np.array(reused_lemmas_count).std(axis=0, ddof=1).flatten()
    ax.fill_between(
        x=thresholds,
        y1=reused_lemma_count_mean - reused_lemma_count_std,
        y2=reused_lemma_count_mean + reused_lemma_count_std,
        alpha=0.2,
    )

    ax2 = ax.twinx()
    non_retr_count_mean = (
        (np.array(non_retr_count) / np.array(non_retr_count)[:, 0, np.newaxis] * 100)
        .mean(axis=0)
        .flatten()
    )
    ax2.plot(thresholds, non_retr_count_mean, color="red")
    non_retr_count_std = (
        (np.array(non_retr_count) / np.array(non_retr_count)[:, 0, np.newaxis] * 100)
        .std(axis=0, ddof=1)
        .flatten()
    ...
    ax2.fill_between(
        x=thresholds,
        y1=non_retr_count_mean - non_retr_count_std,
        y2=non_retr_count_mean + non_retr_count_std,
        alpha=0.2,
        color="red",
    )

    unrelated_count_mean = (
        (np.array(unrelated_count) / np.array(unrelated_count)[:, 0, np.newaxis] * 100)
        .mean(axis=0)
        .flatten()
    )
    ax2.plot(thresholds, unrelated_count_mean, color="orange")
    unrelated_count_std = (
        (np.array(unrelated_count) / np.array(unrelated_count)[:, 0, np.newaxis] * 100)
        .std(axis=0, ddof=1)
        .flatten()
    ...
    ax2.fill_between(
        x=thresholds,
        y1=unrelated_count_mean - unrelated_count_std,
        y2=unrelated_count_mean + unrelated_count_std,
        alpha=0.2,
        color="orange",
    )
    ax2.set_ylabel(
        "% of Non-Retrieved Lemmas Meeting Similarity Threshold in 2 Solutions"
    )

    _, name = os.path.split(output_path)
    plt.title(name)

    # Force alignment between the two axes (i.e.,100% on right aligns with all lemmas on left)
    MARGIN_SCALE = 1.25  # /4
    ax.set_ylim(0, MARGIN_SCALE * reused_lemma_count_mean[0])
    ax2.set_ylim(0, MARGIN_SCALE * 100)

    plt.savefig(os.path.join(output_path, f"lemma_count_sim_thres_deviation.png"))
    plt.close()

    task_with_reuse_count_mean = (
        (
            np.array(task_with_reuse_count)
            / np.array(num_successes)[..., np.newaxis]
            * 100
        )
        .mean(axis=0)
        .flatten()
    )
    plt.plot(thresholds, np.array(task_with_reuse_count_mean))
    plt.xlabel("Similarity Threshold")
    plt.ylabel("Percentage of Solved Tasks using a reused lemma")
    ax = plt.gca()
    ax.axvline(np.average(non_retrieved_sim), color="red", linestyle="--")

    task_with_reuse_count_std = (
        (
            np.array(task_with_reuse_count)
            / np.array(task_with_reuse_count)[:, 0, np.newaxis]
            * 100
        )
        .std(axis=0, ddof=1)
        .flatten()
    ...
    ax.fill_between(
        x=thresholds,
        y1=task_with_reuse_count_mean - task_with_reuse_count_std,
        y2=task_with_reuse_count_mean + task_with_reuse_count_std,
        alpha=0.2,
    )
    ax.set_ylim(0, 100)
    _, name = os.path.split(output_path)
    plt.title(name)
    plt.savefig(os.path.join(output_path, f"task_count_sim_thres_deviation.png"))
    plt.close()
    ...
    _, name = os.path.split(output_path)
    plt.title(name)
    plt.savefig(os.path.join(output_path, f"lemma_count_sim_thres_aggregate.png"))
    plt.close()
    ...
    _, name = os.path.split(output_path)
    plt.title(name)
    plt.savefig(os.path.join(output_path, f"task_count_sim_thres_aggregate.png"))
    plt.close()


# Regex for finding AFP lemmas.
# Looking for "lemma" or "theorem", some characters,
# a colon, the body (which can't include
# the strings 'lemma' or 'theorem', and finally qed)
# Doesn't match all lemmas, doesn't match entire lemmas, but close enough
...
if __name__ == "__main__":
    ...
    for input_path, output_path, unrelated in [
        ...
        (
            ...
            "data_analysis/calc_common_ancestors/exp4_o3mini",
            afp_lemmas,
        ),
        # 4o, experiment 5
        (
            ...
            "data_analysis/calc_common_ancestors/exp5_4o",
            afp_lemmas,
        ),
        # Exp 1, 4o-mini
        (
            ...
            "data_analysis/calc_common_ancestors/exp1_4omini",
            afp_lemmas,
        ),
        ...
        (
            ...
            "data_analysis/calc_common_ancestors/10perc_qwen3",
            afp_lemmas,
        ),
        # Llama 3.1, 100%
        (
            ...
            "data_analysis/calc_common_ancestors/100perc_llama3.1",
            afp_lemmas,
        ),
    ]:
        main(
