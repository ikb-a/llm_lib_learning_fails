...
from autogen import OpenAIWrapper, filter_config
from autogen.llm_config import LLMConfig
from ca import ConversableAgent
...
# def execute_func(name_z123, packages_z123, code_z123, **args):
...
class AgentOptimizer:
    ...
    def _get_response_with_retry(self, messages, max_attempts = 30):
        try:
            return self._client.create(
                    messages=messages, tools=[ADD_FUNC, REVISE_FUNC, REMOVE_FUNC], tool_choice="auto"
                )
        except Exception as e:
            if max_attempts < 0:  # Give up
                raise
            print("Attempting to call OpenAI resulted in:")
            print(f"{e.__class__}: {e}")
            
            import random
            exp_time = min(2**(10-max_attempts), 8) * 15 + 5*random.random()
            print(f"Will sleep {exp_time}s & retry", flush=True)
            import time 
            time.sleep(exp_time)
            return self._get_response_with_retry(messages, max_attempts=max_attempts-1)
    ...
    def step(self):
        ...
        for action_index in range(self.max_actions_per_step):
            ...
            for _ in range(self._max_trials):
                ...
                #     messages=messages, tools=[ADD_FUNC, REVISE_FUNC, REMOVE_FUNC], tool_choice="auto"
                # )
                response = self._get_response_with_retry(messages)
        ...
        for name in remove_functions:
            ...
            register_for_log.append({'action': 'delete', 'target': name})
        for func in incumbent_functions:
            ...
            # })
            # register_for_exector.update({
            #     func.get("name"): {'packages': 
            #                        func.get("packages"), 
            #                        'code': func.get("code"), 
            #                        'lambda': lambda **args: execute_func(
            #          func.get("name"), func.get("packages"), func.get("code"), **args
            #      )}
            ...
            register_for_log.append({'action': 'add', 'target': {'name': func.get('name'),
                                                                 'description': func.get('description'),
                                                                 'code': func.get('code'),
        ...
        return register_for_llm, register_for_exector, register_for_log
            ...
            for action in actions:
                ...
                try:
                    ...
                    if action.function.name == 'remove_function':
                        continue # Remove function OK so far.

                    # print(json.dumps(function_args, indent=2, sort_keys=True))
                    # print()
                    # LLM is very bad at producing the items key, catch it here so the entire
                    # tool call attempt doesn't fail down the line.
                    if "arguments" not in function_args:
                        print("JSON is invalid: no arguments")
                        return False
                    
                    if "name" not in function_args:
                        print("JSON is invalid: no name of function to alter")
                        return False 

                    ConversableAgent._assert_valid_name(function_args.get("name"))

                    if "arguments" in function_args:
                        detailed_args = json.loads(function_args.get("arguments").strip('"'))
                        print(json.dumps(detailed_args, indent=2, sort_keys=True))
                        #print()
                        for detailed_arg in detailed_args.values():
                            if 'type' not in detailed_arg:
                                print('JSON is invalid: no type for arg')
                                return False 
                            if any(x in detailed_arg for x in ['required']):
                                print('JSON is invalid, extra key')
                                return False
                            if 'description' not in detailed_arg:
                                print('JSON is invalid: arg missing description')
                                return False 
                            if detailed_arg['type'] == 'array':
                                if 'items' not in detailed_arg or 'type' not in detailed_arg['items'] or detailed_arg['items']['type'] not in ['string', 'integer', 'object', 'array', 'boolean']:
                                    print('JSON is invalid: item not specified in an array type argument')
                                    return False
                            if 'type' in detailed_arg and detailed_arg['type'] not in ['string', 'integer', 'object', 'array', 'boolean']: # blarg
                                print(f'JSON not valid: bad type {detailed_arg["type"]}')
                                return False
                except Exception as e:
