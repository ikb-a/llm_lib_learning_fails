...
from autogen.io.run_response import AsyncRunResponse, AsyncRunResponseProtocol, RunResponse, RunResponseProtocol
...
from autogen.oai.client import OpenAIWrapper as OAW
from autogen.runtime_logging import log_event, log_function_use, log_new_agent, logging_enabled
from autogen.tools import ChatContext, Tool, load_basemodels_if_needed, serialize_to_str
from autogen.agentchat.agent import Agent, LLMAgent
from autogen.agentchat.chat import (
    ChatResult,
...
from autogen.agentchat.conversable_agent import UpdateSystemMessage, register_function
from autogen.agentchat.conversable_agent import ConversableAgent as CA


class RetryWrappedOpenAIWrapper(OAW):
    def __init__(self, *, config_list = None, **base_config: Any):
        super().__init__(config_list=config_list, **base_config)
        self._remining_retries = 30

    def create(self, **config: Any) -> ModelClient.ModelClientResponseProtocol:
        try:
            response = super().create(**config)
            self._remining_retries = 30
            return response
        except Exception as e:
            if self._remining_retries < 0:  # Give up
                raise
            print("Attempting to call OpenAI resulted in:")
            print(f"{e.__class__}: {e}")
            
            import random
            exp_time = min(2**(10-self._remining_retries), 8) * 15 + 5*random.random()
            print(f"Will sleep {exp_time}s & retry", flush=True)
            import time 
            time.sleep(exp_time)

            self._remining_retries -= 1
            return self.create(**config)


# RENAME TO PREVENT COLLISIONS
def innermost_execute_func(name_z123, packages_z123, code_z123, **args):
    """The wrapper for generated functions."""
    pip_install = (
        f"""print("Installing package: {packages_z123}")\nsubprocess.run(["pip", "-qq", "install", "{packages_z123}"])"""
        if packages_z123
        else ""
    )
    str = f"""
import subprocess
{pip_install}
print("Result of {name_z123} function execution:")
{code_z123}
args={args}
result={name_z123}(**args)
if result is not None: print(result)
"""
    print(f"execute_code:\n{str}")
    result = execute_code(str, use_docker=False, timeout=20)
    if result[0] != 0:
...
# New conversable agent with more understandable function dictionary.
class ConversableAgent(CA):
    ...
    def __init__(
        ...
        # )
        self._function_map = {}  # Change to Dict from name to (param, code)
                ...
                def create_wrapper(update_func: UpdateSystemMessage):
                    def update_system_message_wrapper(
                        ...
                        if isinstance(update_func.content_updater, str):
                            ...
                            sys_message = RetryWrappedOpenAIWrapper.instantiate(
    ...
    @classmethod
    def _create_client(cls, llm_config: Union[LLMConfig, Literal[False]]) -> Optional[OAW]:
        return None if llm_config is False else RetryWrappedOpenAIWrapper(**llm_config)
    ...
    def generate_function_call_reply(
        ...
        if message.get("function_call"):
            ...
            #     raise NotImplementedError("Screw co-routine")
            #     # try:
    ...
    def generate_tool_calls_reply(
        ...
        for tool_call in message.get("tool_calls", []):
            ...
            # if inspect.iscoroutinefunction(func):
            #     raise NotImplementedError("Screw async")
            #     # try:
            #     #     # get the running loop if it was already created
            #     #     loop = asyncio.get_running_loop()
            #     #     close_loop = False
        ...
        if func is not None:
            ...
            if arguments is not None:
                ...
                try:
                    ...
                    if func_name not in func.get("code"):
                        print("WARNING: Function name not in code?")

                    content = innermost_execute_func(func_name, func.get("packages"), func.get("code"), **arguments)
        ...
        if func is not None:
            ...
            if arguments is not None:
                ...
                try:
                    ...
                    content = innermost_execute_func(func_name, func['packages'], func['code'], **arguments)
                    is_exec_success = True
                except Exception as e:
                    content = f"Error: {e}"
        else:
    ...
    def register_function(self, function_map: dict[str, Union[Callable[..., Any]]], silent_override: bool = False):
        ...
        """
        for name, func_descr in function_map.items():
            self._assert_valid_name(name)
            if func_descr is None and name not in self._function_map:
                warnings.warn(f"The function {name} to remove doesn't exist", UserWarning)
    ...
    def update_function_signature(
        ...
        self.client = RetryWrappedOpenAIWrapper(**self.llm_config)
    ...
    def function_map(self) -> dict[str, Callable[..., Any]]:
        ...
        return {n: lambda **args: innermost_execute_func(n, self._function_map['packages'], self._function_map['code'], **args) 
                for n in self._function_map if self._function_map[n] is not None}
